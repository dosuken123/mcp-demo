...
onMounted(async (): Promise<void> => {
  ...
  try {
    ...
    if (window.location.search.includes('code=')) {
      ...
      if (result.success) {
        toolData.value = result.toolData;
        accessToken.value = mcpClient.getAccessToken();
...
<template>
  <div>
    <h2>{{ toolData ? 'Welcome' : 'My MCP Client auth' }}</h2>
    ...
    <!-- Tool List -->
    <div v-if="toolData">
      <h3>Connected to MCP Server</h3>
      
      <div>
        <strong>Tool Information:</strong>
        <pre style="background: #f5f5f5; padding: 10px; border-radius: 4px; margin-top: 10px;">{{ JSON.stringify(toolData, null, 2) }}</pre>
      </div>
